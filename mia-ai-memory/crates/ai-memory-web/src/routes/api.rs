...
use ai_memory_store::{
    ...
    RelatedPage, ScopeName, ScopeResolutionError, SearchExplain, SessionSummary,
...
pub(crate) fn build(state: Arc<WebState>) -> Router {
    Router::new()
        ...
        .route(
            "/workspaces/{workspace}/projects/{project}/search/explain",
            axum::routing::post(explained_search_handler),
        )
        .route("/activity", axum::routing::get(activity_handler))
        .route(
            "/workspaces/{workspace}/projects/{project}/agents",
            axum::routing::get(project_agents_handler),
        )
        .route(
            "/workspaces/{workspace}/projects/{project}/contradictions",
            axum::routing::get(project_contradictions_handler),
        )
        .route(
...
async fn explained_search_handler(
    State(state): State<Arc<WebState>>,
    Path((workspace, project)): Path<(String, String)>,
    Json(request): Json<ExplainSearchRequest>,
) -> Result<Response, Response> {
    let term = request.q.trim().to_owned();
    if term.is_empty() {
        return Ok(Json(Vec::<ApiExplainedSearchHit>::new()).into_response());
    }
    ...
    let query_vec = request.query_vector;
    let provider = request.embedding_provider.unwrap_or_default();
    let model = request.embedding_model.unwrap_or_default();
    let dim = request
        .embedding_dim
        .or_else(|| query_vec.as_ref().and_then(|vector| u32::try_from(vector.len()).ok()))
        .unwrap_or(0);
    if let Some(vector) = query_vec.as_ref() {
        if provider.trim().is_empty() || model.trim().is_empty() || dim == 0 {
            return Err(json_error(
                StatusCode::BAD_REQUEST,
                "embedding_provider, embedding_model and embedding_dim are required with query_vector",
            ));
        }
        if vector.len() != usize::try_from(dim).unwrap_or(usize::MAX) {
            return Err(json_error(
                StatusCode::BAD_REQUEST,
    ...
    for (hit, explain) in hits {
        if let Some(meta) = state
            ...
            .map_err(internal_error)?
        {
            out.push(ApiExplainedSearchHit {
                workspace: meta.workspace_name,
                project: meta.project_name,
                path: meta.path,
                title: hit.title,
    ...
    Ok(with_no_store(Json(out).into_response()))
}

async fn activity_handler(State(state): State<Arc<WebState>>) -> Result<Response, Response> {
    let activity = state
        .reader
        ...
        .map_err(internal_error)?;
    Ok(with_no_store(
        Json(serde_json::json!({ "clients": activity })).into_response(),
    ))
}

async fn project_agents_handler(
    State(state): State<Arc<WebState>>,
    ...
    let agents = state
        .reader
        .session_counts_by_agent(workspace_id, project_id, owner_filter_for(actor), None)
        .await
        .map_err(internal_error)?;
    Ok(with_no_store(
...
async fn project_contradictions_handler(
    State(state): State<Arc<WebState>>,
    Path((workspace, project)): Path<(String, String)>,
) -> Result<Response, Response> {
    let (workspace_id, project_id) = lookup_project(&state, &workspace, &project).await?;
    let edges = state
        .reader
        .contradiction_edges(workspace_id, project_id)
        .await
        .map_err(internal_error)?;
    Ok(with_no_store(
        Json(serde_json::json!({ "contradictions": edges })).into_response(),
    ))
}
...
async fn overview_handler(
    ...
    let detail = state
        ...
        .await
        .map_err(internal_error)?;
    let (contradictions, contradiction_pages) =
        workspace_contradictions(&state, &workspace, query.limit.clamp(1, 100)).await?;
...
async fn workspace_contradictions(
    state: &WebState,
    workspace: &str,
    detail_limit: usize,
) -> Result<(u64, Vec<ApiContradiction>), Response> {
    let projects = state
        .reader
        .list_projects_with_stats_for_workspace(workspace.to_owned())
        .await
    ...
    for project in projects {
        let (workspace_id, project_id) =
            lookup_project(state, workspace, &project.project_name).await?;
        let edges = state
            .reader
            .contradiction_edges(workspace_id, project_id)
            .await
            .map_err(internal_error)?;
        total = total.saturating_add(u64::try_from(edges.len()).unwrap_or(u64::MAX));
        if details.len() < detail_limit {
            let remaining = detail_limit - details.len();
            details.extend(edges.into_iter().take(remaining).map(|edge| ApiContradiction {
...
async fn project_overview_handler(
    ...
    let detail = state
        ...
        .await
        .map_err(internal_error)?;
    let contradiction_edges = state
        .reader
        .contradiction_edges(workspace_id, project_id)
        .await
        .map_err(internal_error)?;
    let contradictions = u64::try_from(contradiction_edges.len()).unwrap_or(u64::MAX);
    let contradiction_pages = contradiction_edges
        .into_iter()
        .take(limit)
        .map(|edge| ApiContradiction {
...
struct ApiHealth {
    ...
    contradiction_pages: Vec<ApiContradiction>,
}
