...
fn stamp_session_origin_map(
    ...
    if let Some(provenance_map) = provenance.as_object_mut() {
        provenance_map.insert(
            "source".into(),
            serde_json::Value::String("session-consolidation".into()),
        );
        provenance_map.insert(
            "session_id".into(),
            serde_json::Value::String(session_id.to_string()),
...
mod tests {
    ...
    async fn single_page_consolidation_stamps_and_preserves_session_origin() {
        ...
        for actor in [actor_named("alice"), actor_named("bob")] {
            ...
            assert_eq!(
                stored.frontmatter["provenance"]["session_id"],
                session.to_string()
            );
            assert_eq!(stored.frontmatter["provenance"]["agent"], "claude-code");
        }
    }

    /// The multi-page provider path stamps every generated page with the
    /// immutable session origin so semantic memories keep their provenance.
    #[tokio::test]
    async fn batch_consolidation_stamps_origin_on_every_generated_page() {
        let tmp = tempfile::tempdir().unwrap();
        let (store, wiki, session, ws, proj) = batch_fixture(tmp.path()).await;
        ...
        assert_eq!(
            session_page.frontmatter["provenance"]["source"],
        ...
        let concept = wiki
            ...
            .unwrap();
        assert_eq!(concept.frontmatter["session_id"], session.to_string());
        assert_eq!(concept.frontmatter["agent"], "claude-code");
        assert_eq!(
            concept.frontmatter["provenance"]["session_id"],
            session.to_string()
        );
        assert_eq!(concept.frontmatter["provenance"]["agent"], "claude-code");
    }
