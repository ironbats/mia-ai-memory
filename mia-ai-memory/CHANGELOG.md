...
- Added an optional React Cognitive Console with a PostgreSQL observability
  read-model, neural memory graph, end-to-end session provenance, memory-health
  diagnostics and optimization recommendations, agent/handoff activity, immutable
  memory-version time travel, and explained hybrid retrieval. The integration is
  isolated behind the Docker Compose `cognitive` profile so the default ai-memory
  runtime and Markdown/SQLite source-of-truth path remain unchanged.

### Changed
- Split local Cognitive Console runtime ownership: the backend Compose now starts only ai-memory, PostgreSQL, and Cognitive API, publishes the Cognitive API on loopback for the separate React repository, and allows the internal `ai-memory` Docker hostname through the core host guard.
