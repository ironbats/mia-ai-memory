...
- Start the backend stack with `./script/run-local.sh`. It launches only the
  ai-memory core, PostgreSQL cognitive read-model, and Cognitive API. The React
  Cognitive Console is maintained and started from its separate frontend
  repository. See [`docs/cognitive-console.md`](docs/cognitive-console.md).
- The Cognitive API also exposes an isolated Agent & MCP Registry. It stores
  generic agent profiles, MCP server definitions and reusable credentials in
  PostgreSQL while keeping secret values encrypted with AES-256-GCM and out of
  API responses. The original ai-memory capture, Markdown/OKF and SQLite paths
  remain unchanged.
