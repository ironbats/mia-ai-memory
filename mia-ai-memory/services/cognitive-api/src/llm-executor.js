...
const executeCursor = async ({ agent, model, secrets, systemPrompt, history, userPrompt, state, saveState }) => {
  ...
  if (!externalAgentId) {
    const conversationMode = String(cursor.conversationMode || "agent").toLowerCase()
    const payload = {
      prompt: { text: promptText },
      repos: [{ url: repositoryUrl, startingRef: String(cursor.startingRef || "main") }],
      mode: ["agent", "plan"].includes(conversationMode) ? conversationMode : "agent",
      autoCreatePR: cursor.autoCreatePR === true
    }
    ...
    const conversationMode = String(cursor.conversationMode || "agent").toLowerCase()
    if (["agent", "plan"].includes(conversationMode)) payload.mode = conversationMode
